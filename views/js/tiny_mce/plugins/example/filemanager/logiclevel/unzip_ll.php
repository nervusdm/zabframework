...
$zip_name = empty($path) ? $root_path.$zip_file_name : $root_path.$path."/".$zip_file_name;
...
function unzipFile($root_path,$path,$zip_type,$zip_name,$main_folder_name,$overwrite,$selected_files) {
	...
	$tmp_dir_name = $root_path.$path.'/.__unzip'.$zip_name.$_GET['PHPSESSID'];
	File::deleteFile($tmp_dir_name);
	
	if(File::createFile($tmp_dir_name,'dir'))
		if(File::unzipFile($zip_type,$root_path.$path.'/'.$zip_name,$selected_files,$tmp_dir_name.'/'.$main_folder_name)) {
			$files_error = array();
			if(!empty($main_folder_name)) {
				if(!File::moveFile($tmp_dir_name.'/'.$main_folder_name,$root_path.$path.'/'.$main_folder_name,$overwrite))
					$files_error = array($root_path.$path.'/'.$main_folder_name);
			}
			else {
				$files_list = File::getDirFiles($tmp_dir_name);
				for($i = 0; $i < count($files_list); ++$i)
					if(!File::moveFile($files_list[$i][7],$root_path.$path.'/'.$files_list[$i][0],$overwrite))
...
function updateDirlist($root_path,$path,$main_folder_name,$selected_files,$files_list) {
	$zip_dirname = empty($path) ? $root_path : $root_path.$path."/";
