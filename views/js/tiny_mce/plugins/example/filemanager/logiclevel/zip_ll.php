...
function zipFile($root_path,$path,$zip_type,$zip_name,$selected_files,$main_folder_name,$overwrite) {
	$tmp_dir_name = $root_path.$path.'/.__zip_'.$zip_name.'_'.$zip_type.'_'.$_GET['PHPSESSID'];
	$zip_file_name = $zip_type == "gz" ? $zip_name.".tar.".$zip_type : $zip_name.".".$zip_type;
	
	if(File::createFile($tmp_dir_name,'dir'))
		if(File::zipFile($zip_type,$tmp_dir_name.'/'.$zip_name,$selected_files,$main_folder_name,$root_path.$path))
			if(File::moveFile($tmp_dir_name.'/'.$zip_file_name,$root_path.$path.'/'.$zip_file_name,$overwrite)) {
