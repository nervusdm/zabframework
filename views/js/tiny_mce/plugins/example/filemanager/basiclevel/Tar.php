...
require_once './../basiclevel/PEAR.php';
...
class Archive_Tar extends PEAR
{
    ...
    */
    ...
    {
        return $this->addModify($p_filelist, $p_add_dir, $p_remove_dir);
    }
    // }}}

    // {{{ extract()
    function extract($p_path='', $p_remove_path='', $p_filelist=false)
    {
		return is_array($p_filelist) ? $this->extractList($p_filelist, $p_path, $p_remove_path) : $this->extractModify($p_path, $p_remove_path);
    }
    // }}}
    ...
    function _addList($p_list, $p_add_dir, $p_remove_dir)
    {
      ...
      foreach ($p_list as $v_filename) {
        ...
        if (@is_dir($v_filename)) {
			...
			$full_dirname_path = $v_filename != "." ? $v_filename.'/' : '';
            while (false !== ($p_hitem = readdir($p_hdir))) {
                if (($p_hitem != '.') && ($p_hitem != '..') && CONFIGFILE::isValid($full_dirname_path.$p_hitem)) {
                    if ($v_filename != ".")
